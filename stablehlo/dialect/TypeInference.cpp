...
bool allQuantized(ArrayRef<Type> range) {
  return llvm::all_of(range, [&](Type val) {
    return val.cast<ShapedType>().getElementType().isa<mlir::quant::QuantizedType>();});
}
...
bool allPerAxisQuantized(ArrayRef<Type> range) {
  return llvm::all_of(range, [&](Type val) {
    return val.cast<ShapedType>().getElementType().isa<mlir::quant::UniformQuantizedPerAxisType>();});
}
...
LogicalResult verifyConvolutionOp(
  ...
  if(!allQuantized(typeEntries)) {
    // convolution_c28
    return emitOptionalError(location, "not all of operands and result are quantized");
  }
  ...
  if (lhsQType.getStorageType() != rhsQType.getStorageType())
    return emitOptionalError(location, "mismatched operand storage types ", lhsQType.getStorageType(),
            " and ", rhsQType.getStorageType(), "");
  // convolution_c30
  ...
  if (expressedType != rhsQType.getExpressedType() ||
      ...
      return emitOptionalError(location, "mismatched operands and result expressed types");
  ...
  if(!allPerAxisQuantized(typeEntriesPerAxis)){
    return emitOptionalError(location, "operand and result are of mixed per_tensor and per_axis quantized tensor type");
  }
  ...
  if(rhsQPAType &&
      ...
      return emitOptionalError(location, "mismatched kernel_output_feature_dimension ",
      kernelOutputFeatureDimension, " and operand quantized dimension ", rhsQPAType.getQuantizedDimension(), "");
  // convolution_c33
  if(resultQPAType &&
      resultQPAType.getQuantizedDimension() != kernelOutputFeatureDimension)
      return emitOptionalError(location, "mismatched kernel_output_feature_dimension ", kernelOutputFeatureDimension,
            " and result quantized dimension ", resultQPAType.getQuantizedDimension(), "");
